...
type RouteMode = 'idle' | 'selecting' | 'active'
...
export default function MapView({ unlockedCheckpoints }: MapViewProps) {
  ...
  const activeRoute = useMemo(() => {
    if (routeMode !== 'active' || !routeFrom || !selectedPOI || routeSteps.length === 0) return null
  ...
  const handleSelectPOI = (poi: FloorPlanPOI | null) => {
    if (routeMode === 'active') {
      handleClearRoute()
    }
    setSelectedPOI(poi)
    ...
    <div className="flex flex-col w-full h-full bg-background overflow-hidden font-sans">
      ...
      <div className="flex-1 relative w-full max-w-md mx-auto overflow-hidden bg-surface-dim">
        ...
        {routeMode === 'active' && routeSteps.length > 0 && (
          <div className="absolute top-16 left-4 right-4 z-20 max-h-[120px] overflow-y-auto hide-scrollbar">
            <div className="bg-white/95 dark:bg-[#0d1420]/95 glass-panel rounded-xl p-3 shadow-md border border-outline-variant/15">
              <div className="flex items-center gap-1.5 mb-2 text-[10px] font-extrabold text-[#F7931E] uppercase tracking-wider">
        ...
        {selectedPOI && (
          <div className="absolute bottom-0 left-0 right-0 z-[400] bg-white/95 dark:bg-[#0d1420]/95 backdrop-blur-xl rounded-t-[28px] shadow-[0px_-8px_30px_rgba(44,62,115,0.15)] border-t border-outline-variant/20 animate-in slide-in-from-bottom duration-300 pointer-events-auto">
            <div
              ...
              onClick={() => {
                if (routeMode === 'active') handleClearRoute()
                setSelectedPOI(null)
              }}
            >
            ...
            <div className="px-6 pb-8 text-on-surface dark:text-inverse-on-surface select-none">
              {routeMode === 'selecting' ? (
                <>
                  <h3 className="font-extrabold text-sm text-primary dark:text-inverse-primary mb-4">
                    Trazar ruta
                  </h3>
                  ...
                  <div className="space-y-3 mb-5">
                    <div>
                      ...
                      <select
                        ...
                        className="w-full p-3 rounded-xl bg-gray-50 dark:bg-white/5 border border-outline-variant/15 text-xs font-bold text-primary dark:text-inverse-primary outline-none"
                      >
                    ...
                    <div>
                      ...
                      <div className="w-full p-3 rounded-xl bg-[#2C3E73]/5 dark:bg-white/5 border border-outline-variant/15 text-xs font-bold text-primary dark:text-inverse-primary">
                        {selectedPOI.label}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-3">
                    ...
                    <button
                      ...
                      className="px-5 py-3 rounded-xl border border-outline-variant/20 text-xs font-bold text-gray-500 hover:bg-gray-50 dark:hover:bg-white/5 active:scale-95 transition-all cursor-pointer bg-transparent"
                    >
                      Cancelar
                    </button>
                  </div>
                ...
                <>
                  <div className="flex justify-between items-start mb-4">
                    <div className="min-w-0 flex-1">
                      <span className="inline-flex items-center gap-1 text-[9px] font-extrabold uppercase tracking-wider mb-1 px-2.5 py-0.5 rounded-full bg-[#2C3E73]/10 text-[#2C3E73] dark:bg-white/10 dark:text-gray-300">
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-4">
                    <div className="bg-gray-50 dark:bg-white/5 p-3 rounded-2xl border border-outline-variant/10">
                      ...
                      <p className="text-xs font-extrabold text-primary dark:text-[#fee269] mt-0.5 leading-none">
                  ...
                  <div className="flex gap-3">
                    ...
                    <button
                      ...
                      className="w-12 h-12 flex items-center justify-center border border-outline-variant/20 text-[#2C3E73] dark:text-[#fee269] hover:bg-gray-50 dark:hover:bg-white/5 rounded-xl active:scale-95 transition-all cursor-pointer shadow-sm"
                    >
        ...
        {/* Legend */}
        <div className="absolute top-20 right-4 z-10 w-32 glass-premium rounded-2xl p-3 border border-outline-variant/25 shadow-md select-none pointer-events-none text-on-surface">
          ...
          <ul className="space-y-2">
            <li className="flex items-center gap-2 text-[9px] font-bold text-gray-500 dark:text-gray-300 uppercase leading-none">
              <span className="w-2.5 h-2.5 rounded-full bg-[#fee269] border border-white dark:border-[#0d1420] shadow-sm"></span> Proyecto
            </li>
            <li className="flex items-center gap-2 text-[9px] font-bold text-gray-500 dark:text-gray-300 uppercase leading-none">
              <span className="w-2.5 h-2.5 rounded-full bg-[#22C55E] border border-white dark:border-[#0d1420] shadow-sm"></span> Completado
            </li>
            <li className="flex items-center gap-2 text-[9px] font-bold text-gray-500 dark:text-gray-300 uppercase leading-none">
              <span className="w-2.5 h-2.5 rounded-full bg-[#2C3E73] border border-white dark:border-[#0d1420] shadow-sm"></span> Entrada
            </li>
          </ul>
        </div>
      </div>
